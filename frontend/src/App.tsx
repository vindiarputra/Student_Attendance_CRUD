...
const API_URL = "http://localhost/Student_Attendance_CRUD/backend/koneksi.php"; // Replace with your backend URL
...
export default function StudentAttendanceCRUD() {
    ...
    const handleChange = (name: keyof AttendanceFormData, value: string | Date | File) => {
        if (name === "proof_image" && value instanceof File) {
                ...
                <Card>
					...
					<form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            {formData.status === "Sick" && (
                               <div className="space-y-2">
							   <Label htmlFor="proof_image">Proof Image (if sick)</Label>
							   <div className="flex items-center">
								   <label
									   htmlFor="proof_image"
									   className="bg-gray-200 text-gray-800 px-4 py-2 rounded-md cursor-pointer hover:bg-gray-300 transition-colors"
								   >
									   Choose File
								   </label>
								   <span className="ml-2 text-sm text-gray-400">
									   {formData.proof_image ? "File selected" : "No file chosen"}
								   </span>
							   </div>
									<Input
										id="proof_image"
